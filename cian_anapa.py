...
for y in ids:
        ...
        for i in items:
            try:
                if i['building']['deadline']['isComplete'] == True:
                    complete_date = "Дом сдан"
                else:
                    complete_date = f"Cдача ГК: {i['building']['deadline']['quarterEnd']}"
            except:
                complete_date = ''
            try:
                url = i['fullUrl']
            ...
            print(
                f"{complete_date}, URL {url}, тип жилья {flat_type}, {price}, {jk}, отделка {decoration}, отделка 2 {decoration2}, {adress}, {developer}, {added}, {rooms}, {area}, {city}, От застройщика: {is_biulder}, Этаж: {floor}")
            result = [complete_date, url, flat_type, full_info, price, jk, decoration, decoration2,
                      adress, house, developer, date, added, rooms, area, city, is_biulder, floor]
            flats.append(result)
    ...
    filename = f"{city}_{jk}_{current_date}.xlsx"

    # Полный путь к файлу
    file_path = os.path.join(folder_path, filename)

    df = pd.DataFrame(flats, columns=["Сдача корпуса",
                                  "Ссылка",
                                  "Тип жилья",
                                  "Описание",
                                  "Цена",
