...
while counter < 19:
    ...
    for y in range(1, len(flats_soup)):
        ...
        adress = ''
        eskrou = ''
        korpus = td_elements[0].text.strip()
        konstruktiv = ''
        klass = ''
        srok_sdachi = ''
        srok_sdachi_old = ''
        ...
        room_count = td_elements[1].text.strip()

        area = float(td_elements[4].text.strip())
        price_per_metr = ''
        old_price = td_elements[6].text.strip().replace(' ', '')

        discount = ''
        price_per_metr_new = ''
        price = int(td_elements[7].text.strip().replace(' ', ''))
        section = ''
        floor = td_elements[2].text.strip()
        flat_number = ''
...
base_path = r"C:\PycharmProjects\SeleniumParcer\Ар Ди Ай"

folder_path = os.path.join(base_path, str(current_date))
if not os.path.exists(folder_path):
    os.makedirs(folder_path)

filename = f"{developer}_{project}-3_{current_date}.xlsx"
